...
/** 连接 Electron 托管或本进程自动拉起的 Admin 桥接（固定 localhost 端口） */
export async function ensureAdminBridge(): Promise<string> {
  ...
  if (cachedBaseUrl && (await healthCheck(cachedBaseUrl))) {
    ...
    if (!jarPath) {
      throw new Error(
        '未找到内置 RocketMQ Admin 桥接 JAR。\n' +
          '请先启动 MiddleTool 桌面端（由 Electron 托管 Admin），或在开发环境执行：npm run rocketmq-mcp:java:build'
      )
    }
